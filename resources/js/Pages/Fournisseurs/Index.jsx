...
import { Edit2, Trash2, Plus, Search, AlertTriangle, Truck, History } from 'lucide-react';

export default function Index({ auth, fournisseurs, filters }) {
    ...
    const handleDeleteConfirm = () => {
        if (fournisseurToDelete) {
            router.delete(route('fournisseurs.destroy', fournisseurToDelete.id), {
    ...
    return (
        <AuthenticatedLayout
            ...
            header={
                <div>
                    <h2 className="font-bold text-2xl text-gray-900 leading-tight tracking-tight">Fournisseurs</h2>
                    <p className="text-sm text-gray-500 mt-1">Gérez vos fournisseurs et leurs coordonnées.</p>
                </div>
            }
        >
                                    ...
                                    <tr>
                                        <td colSpan="4" className="px-6 py-12 text-center">
                                            <div className="flex flex-col items-center justify-center text-gray-400">
                                                ...
                                                <p className="text-base font-medium text-gray-500">Aucun fournisseur trouvé</p>
                                        ...
                                        <tr key={f.id} className="hover:bg-gray-50/50 transition-colors group">
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center">
                                                    ...
                                                    <div className="ml-3">
                                                        <p className="text-sm font-semibold text-gray-900">{f.nom}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-gray-600 font-medium">{f.email || '---'}</td>
                                            <td className="px-6 py-4 whitespace-nowrap text-gray-600">{f.telephone || '---'}</td>
                                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div className="flex justify-end items-center gap-2">
                                                    <Link 
                                                        href={route('fournisseurs.achats', f.id)} 
                                                    ...
                                                    <Link 
                                                        href={route('fournisseurs.edit', f.id)} 
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-x-hidden overflow-y-auto">
                    ...
                    <div className="relative bg-white rounded-2xl shadow-xl max-w-md w-full p-6 animate-in fade-in zoom-in-95 duration-200">
                        <div className="flex items-start gap-4">
                            ...
                            <div>
                                <h3 className="text-lg font-bold text-gray-900">Supprimer le fournisseur</h3>
                                <p className="mt-2 text-sm text-gray-500 leading-relaxed">
                                    Êtes-vous sûr de vouloir supprimer <strong className="text-gray-900">{fournisseurToDelete?.nom}</strong> ? Cette action est irréversible.
                                </p>
                            </div>
                        </div>
