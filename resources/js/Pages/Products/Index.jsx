...
import { Edit2, Trash2, Plus, Search, AlertTriangle, Package } from 'lucide-react';
...
export default function Index({ auth, products, filters }) {
    ...
    return (
        <AuthenticatedLayout
            ...
            header={
                <div>
                    <h2 className="font-bold text-2xl text-gray-900 leading-tight tracking-tight">Produits & Services</h2>
                    <p className="text-sm text-gray-500 mt-1">Gérez votre catalogue de produits et de services.</p>
                </div>
            }
        >
            <Head title="Produits" />
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-100 text-sm text-left">
                            <thead className="bg-gray-50/50">
                                <tr>
                                    <th className="px-6 py-4 font-semibold text-gray-500 text-xs uppercase tracking-wider">Nom du produit</th>
                                    <th className="px-6 py-4 font-semibold text-gray-500 text-xs uppercase tracking-wider">Description</th>
                                    <th className="px-6 py-4 font-semibold text-gray-500 text-xs uppercase tracking-wider">Prix (HT)</th>
                                    ...
                                    <tr>
                                        <td colSpan="4" className="px-6 py-12 text-center">
                                            <div className="flex flex-col items-center justify-center text-gray-400">
                                                ...
                                                <p className="text-base font-medium text-gray-500">Aucun produit trouvé</p>
                                        ...
                                        <tr key={product.id} className="hover:bg-gray-50/50 transition-colors group">
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center">
                                                    <div className="h-9 w-9 rounded-lg bg-indigo-50 flex items-center justify-center text-indigo-600 ring-1 ring-indigo-100 mr-3">
                                                        <Package className="w-4 h-4" />
                                                    </div>
                                                    <span className="font-semibold text-gray-900">{product.name}</span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 text-sm text-gray-600 max-w-xs truncate" title={product.description}>
                                                {product.description || '---'}
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-semibold">
                                                {parseFloat(product.price || 0).toLocaleString('fr-FR', { minimumFractionDigits: 2 })} DH
                                            </td>
                        ...
                        <div className="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p className="mt-2 text-sm text-gray-500 leading-relaxed">
                                    Êtes-vous sûr de vouloir supprimer <strong className="text-gray-900">{productToDelete?.name}</strong> ? Cette action est irréversible.
                                </p>
                            </div>
                        </div>
